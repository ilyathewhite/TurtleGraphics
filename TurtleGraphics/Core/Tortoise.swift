...
public class Turtle {
    ...
    public init(_ name: String? = nil) {
        self.name = name ?? Turtle.randomName()
    }
    ...
    public func setPosition(_ position: Vec2D) {
        ...
        delegate?.turtleDidChangePosition(uuid, state)
    }
    ...
    public func setHeading(_ heading: Double) {
        ...
        delegate?.turtleDidChangeHeading(uuid, state)
    ...
    // MARK: - [Motion] Tell turtle's state
    ...
    public func penDown() {
        if !state.pen.isDown {
            ...
            delegate?.turtleDidChangePen(uuid, state)
        }
    }
    ...
    public func endFill() {
        delegate?.turtleDidRequestToFill(uuid, state)
        state.fillPath = nil
    }
    ...
    public func reset() {
        state = TurtleState()
        delegate?.turtleDidRequestToClear(uuid, state)
        delegate?.turtleDidInitialized(uuid, state)
    }
    ...
    // MARK: - [Turtle state] Visiblity

    public func showTurtle() {
        if !state.isVisible {
            state.isVisible = true
            delegate?.turtleDidChangeShape(uuid, state)
        }
    }
    ...
    var state: TurtleState = TurtleState()

    weak var delegate: TurtleDelegate?
